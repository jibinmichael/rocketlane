...
| `status` (question answered) | A question with a target and no mission is **answered, not executed**: "Here's what stands between you and **{target}**:" followed by the hop-by-hop chain, then "The first thing to act on is {action}." · "Want me to take it from here? I'll resolve the chain and verify each step." with one chip, `Yes, complete it`, that sends "Complete {target}" as the next turn. Nothing blocked: "Nothing is blocking **{target}** right now." · "Governance is satisfied. Want me to complete it?" Already complete: "**{target}** is already complete. Nothing is blocking it." Blockers are traced read-only from the live graph; no plan, no write. |
| `clarification` (typo) | A target the grounding could not place gets the nearest workspace names as chips: "I couldn't find "{query}". Did you mean one of these?" · chips carry the corrected utterance and send it as the next turn. No match: "I couldn't find "{query}" among the projects and tasks in this workspace." · "Name a project or task and I'll take it from there." |
| `action_request.input` | Three lines, from the step's `RequiredInput`: reason from policy evidence "**{node}** has no logged time. {Policy} requires {field} before completion." · the question "How many hours should I log for **{node}**?" · the authorized actor "{node} is assigned to {assignees}; I'll record the hours as yours, {actor}." (or "I'll record them as your hours, {actor}."). **No field, no button: the composer is the input.** When more than one task in the chain still needs hours, a fourth line says so and offers "2 hours each": a standing answer the runtime applies to every later ask in this mission, one engine input at a time, each with its own receipt "Logged 2 hours by {actor} · same for each". Values arrive as "2 hours", "2h", "2"; once answered, the ask freezes into the thread above the answer with the receipt "Logged 2 hours by {actor}", exactly like a button decision. Invalid values re-state the ask: "I need a number of hours for **{node}**, for example 2 or 1.5. Nothing has been logged." Empty input is not a turn. |
...
| `resumed` | "Got it. I'll recheck the current state, then continue." · only if the world moved: "Resuming requires a course correction. The project changed while this mission was paused. I rechecked the current state and updated the remaining plan." Withdrawn before it took effect: "Got it. The pause was withdrawn before it took effect; I'm continuing." No "Would you like me to continue?" |
...
- **Chrome:** one 48px header: Acme mark + "Acme" left; the acting user's name right, opening a small workspace dialog (acting as · project data · simulate the outside world). No module navigation. Below it, on a mission, one 52px band: goal left · current activity while working ("Preparing mission", "Checking governance", "In flight · Updating **{node}**", "Verifying", "Course correction", "Pausing · finishing the current update") · state chip right · indeterminate hairline while working, solid on landing. **No operation counts anywhere in the chrome.** Composer pinned bottom. No third pinned region.
- **Home:** a peer-agent landing, vertically centred (docs/design/visual-direction.md): `AgentPresence` (60px) · "Your projects are already moving. I'll help keep them on course." · "Check governance, trace blockers, make authorized changes, and verify the result." · composer with rotating placeholder ("State an outcome." → "Complete {first project}" → "What's blocking {first project}?", Tab fills a suggestion) · "AI can make mistakes. Consequential changes are verified before they're treated as complete." · four quick actions that map to real capabilities (complete the first project · what's blocking it · complete my projects · test with project data); the first three fill the composer, the fourth opens the upload panel in place ("Project data loaded. I found {n} projects and {m} tasks…", records needing attention listed, never dropped) · "Previous missions": outcome icon · goal · "{state} · {buckets}" · relative time; a row reopens the persisted mission.
- **Thread rows (ClickUp Brain):** every turn is avatar · name · time, text under the name. The user's avatar is initials on ink; the agent's is `AgentPresence` (28px) and the name is "Governance Agent". Observable work reads as the current step while live with the step list open as a dot spine (hairline, one dot per step, current dot solid); once done it folds to "Finished in N steps ▸". A settled answer ends with a feedback row (helpful · not helpful · copy, local to the session) and follow-up chips that are real capabilities: view the activity log, what's blocking the first project, complete all my projects. Never a dead end.
- **Header:** full bleed. Left: Acme mark · "/" · the mission title as a history dropdown ("Missions" on the home) grouped Today / Yesterday / weekday, needs-you rows flagged, "New mission" first. Right: the acting user opening the workspace dialog. Below it on a mission a 40px status strip: the current activity while working, the state chip right, a vibrant hairline while in flight.
- **Home:** presence (56px) · greeting 20/600 · one support line · composer with rotating placeholder and Tab-fill · trust line (11px) · "Suggested" rows (icon · one line, no cards, no sublines) · "Recent" rows. On send the greeting lifts away and the composer settles to the bottom where the mission page keeps it.
- **Upload:** a modal ("Test with your project files"): drop zone, two file rows (name · Ready · size, removable), "Load and test". Opened from the sparkle control inside the composer ("Test any project files") or the fourth suggested row.
- **Activity is a capability of the mission, not a destination.** "View activity" (landing block, follow-up chip) opens `MissionActivityPanel` over the conversation: this mission's events in order, square markers toned by kind, each row label · refs → chain · reason · verified. "Show audit detail" adds event type, actor, targets, every structured detail key and the event id per row. Esc or the backdrop closes it and the conversation is where it was. There is no `/activity` route and no activity table. Audit data stays complete in the event log; only the presentation is contextual.
- **Text vibrance:** the streaming label shimmers in colour only (no background). Suggested rows carry the four stops on the icon always and on the text while hovered. No uppercase text anywhere in the product.
- **Composer:** one component on the home and on a mission. White surface on the off-white canvas, 20px radius, hairline ring + soft lift, autogrow to 180px. Attach (paperclip) inside on the left opens "Test with project data" in place. The trailing control swaps with a spring: Send (arrow, filled when there is text) → "Pause" pill while in flight (Esc does the same; "Esc pauses" reads beside the attach) → "Resume" pill while paused. No mic, no skills, no mode chips.
- **Presence:** `AgentPresence` is the agent's only face: two square eyes that blink every 5s, a neutral grey conic halo that turns only while working, fades on landing, and rests the eyes while paused. 60px above the home headline; 16px on the thread's working row beside the current session label ("Checking governance", "Updating **{node}**", "Verifying"). Never a mood, never a colour, never an emoji.
- **Palette:** neutral grey only; no accent. State appears as a coloured dot inside a grey chip and as tone-coloured lucide icons. Font: Avenir Next with the system stack as fallback. Dark mode tokens defined, not reviewed.
