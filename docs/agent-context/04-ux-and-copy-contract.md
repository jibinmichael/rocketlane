...
- **Home:** "Your projects are already moving. I'll help keep them on course." · "Check governance, trace blockers, make authorized changes, and verify the result." · composer · "AI can make mistakes. Consequential changes are verified before they're treated as complete." · "Test with project data" expands the upload panel in place ("Project data loaded. I found {n} projects and {m} tasks…", records needing attention listed, never dropped) · "Previous missions": outcome icon · goal · "{state} · {buckets}" · relative time; a row reopens the persisted mission.
