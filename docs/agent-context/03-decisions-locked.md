...
**D-30 · Undo is a mission.** A landed mission offers "Undo this mission" on its landing block. Undo starts a new mission (`origin: "undo"`, `reverts: <source id>`) whose plan reverses the source's verified writes newest first, restoring the exact status each forward step recorded in `PlanStep.before`, and removing the time entry the source created (found by its correlation id). Reverse steps use `REVERTED` / `TIME_REMOVED` transitions and the `revert_task_status` / `revert_project_status` / `remove_time_entry` write commands, reachable only from an undo plan. Same loop: permission (whoever could make the write can unmake it) → governance (P1–P4 report "not triggered") → write → verify by re-read → evaluation. Reopening a project is high impact and asks first. The source mission's record is never edited. This does not weaken D-21: scope reduction still never reverts; undo is an explicit human decision with its own evidence trail. *Why:* a high-ticket enterprise action must be reversible without hiding the consequence; a reverse plan derived from verified writes is the only honest way to do it.
