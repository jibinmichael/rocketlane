...
| 03 | Domain model | Gate 1: same code builds `WorkspaceGraph` from `rocketlane-export` and `cascading-conflicts` fixtures; unit tests on indexes | todo | |
...
| 18 | CSV ingestion | Gate 8: both shapes ingest; report lists rejects/warnings/findings; real export anomalies surfaced | todo | |
